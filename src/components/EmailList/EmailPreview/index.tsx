...
export const EmailPreview = ({
  ...
  return (
    <div
      className="group grid grid-cols-[15%_75%_10%] hover:bg-blue-100 p-2 rounded-md cursor-pointer text-lg items-center"
      onClick={handleViewClick}
    >
      <div className="flex space-x-3">
        <div>{icon}</div>
        <div
          className={`${isRead ? "text-gray-900" : "font-semibold text-black"}`}
        >
          {from}
        </div>
      </div>
      <div className="flex space-x-4 pr-10">
        <div
          className={`whitespace-nowrap ${isRead ? "text-gray-800" : "text-black font-semibold"}`}
        >
          {subject}
        </div>
